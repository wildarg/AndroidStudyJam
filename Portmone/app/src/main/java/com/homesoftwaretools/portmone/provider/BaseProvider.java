...
public abstract class BaseProvider extends ContentProvider {
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        values = onInsert(tableName, values);
    ...
    protected abstract void onDelete(String tableName, Uri uri);
        ...
        try {
            ...
            for (ContentValues val: values) {
                ...
                if (val.containsKey("timestamp")) {
                    long time = val.getAsLong("timestamp");
                    String date = FormatUtils.DRAWER_DATE_FORMAT.format(new Date(time));
                }
                if (id < 0)
                    throw new SQLiteException("error insert into uri " + uri);
                cnt++;
    ...
    protected abstract void onUpdate(String tableName, ContentValues values);
}
