...
public class UpdateExpenseAsyncTask extends ProgressAsyncTask<Expense, Void, Void> {

    private final ContentResolver resolver;
    private final Uri uri;

    public UpdateExpenseAsyncTask(Context context, Long id) {
        super(context);
        resolver = context.getContentResolver();
        uri = Uri.withAppendedPath(PortmoneContract.Expenses.CONTENT_URI, String.valueOf(id));
    }
    ...
    protected Void doInBackground(Expense... params) {
        ...
        resolver.update(uri, expense.getValues(), null, null);

        String where = PortmoneContract.ExpenseTags.EXPENSE_ID + " = ?";
        String[] args = new String[] {uri.getLastPathSegment()};
        resolver.delete(PortmoneContract.ExpenseTags.CONTENT_URI, where, args);
